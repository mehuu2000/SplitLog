...
## 16. 次フェーズ方針（Split加算方式の切替）

最終更新: 2026-02-21

### 16.1 目的

- Splitの経過時間加算を、用途に応じて
  - ラジオ方式（単一加算）
  - チェック方式（複数分配加算）
  の2種類から選べるようにする
...
1. ラジオ方式（既存）
   - 選択中Split 1件のみが加算対象
   - 1秒ごとに1秒加算（実装上は差分時間を単独加算）

2. チェック方式（新規）
   - チェックONのSplit全件が加算対象
   - 差分時間 `delta` をON件数 `n` で等分し、各Splitへ `delta / n` 加算
   - 合計加算量は常に `delta` を維持
   - 全チェック解除は不可（最低1件ONを強制）

### 16.4 Splitボタン押下時（チェック方式）

- 既存のチェック状態は変更しない
- 新規作成されたSplitを追加でチェックONにする
- 例:
  - 押下前: `Split1, Split2` がON
  - 押下後: `Split1, Split2, Split3` がON

### 16.5 実装方針（データ/ロジック）

1. セッション状態に加算対象集合（例: `activeLapIDs`）を導入
2. ラジオ方式では `activeLapIDs` を単一運用
3. チェック方式では `activeLapIDs` を複数運用
4. 加算処理は `Date` 差分ベースのまま分配ロジックに置換
5. 表示秒の丸め補正（合計整合）は既存ロジックを維持/拡張

### 16.6 UI方針

- ラジオ方式: 現状UIを維持
- チェック方式: チェックON/OFF可能なUIに切替
- どちらの方式でも、既存の編集・メモ・サマリー機能は維持する
...
- 小数分配と表示丸めの組み合わせにより、未選択Splitの表示揺れ（`n <-> n+1`）が発生しやすい
- 実装複雑性を下げ、挙動を説明しやすくするため、Split加算を「整数秒配布」に一本化する

### 17.2 確定仕様

1. 全体経過時間
   - 全体経過は従来どおり `Date` 差分ベース（小数を内部保持）
   - 停止/再開などの挙動は既存仕様を維持

2. Split保持値
   - 各Splitは整数秒のみ保持する（小数Split時間は持たない）

3. 秒配布トリガー
   - `distributedWholeSeconds = floor(elapsedSession)` を基準に配布
   - `delta = distributedWholeSeconds - lastDistributedWholeSeconds`
   - `delta` 回だけ `+1秒` 配布する（タイマー発火回数は基準にしない）

4. ラジオ形式
   - `delta` 回すべてを「現在選択中Split」に配布

5. チェック形式
   - 配布対象はチェックON Split配列（順序は `lap.index` 昇順で固定）
   - `delta` 回をラウンドロビンで1秒ずつ配布
   - 例: `0 -> 1 -> 2 -> ... -> last -> 0 -> ...`
...
2. チェック解除
   - 最低1件ONを強制（全解除不可）
   - 配列削除位置に応じて `distributionCursor` を補正
     - 削除index < cursor: `cursor -= 1`
     - 削除index == cursor: `cursor = cursor % newCount`

3. 選択切替（ラジオ/チェック）
   - チェック形式でも `selectedLapID` は別途保持（編集対象・フォーカス用）
   - ラジオへ戻した際は `selectedLapID` を加算対象に使う

### 17.4 永続化追加項目

- `lastDistributedWholeSeconds`
  - 全体経過のうち「どこまでSplitへ配布済みか」
- `distributionCursor`
  - チェック形式で次に配布する対象index

これにより、再起動後も配布の連続性を維持できる。

### 17.5 期待効果

- 小数分配起因の表示不整合を大幅に削減
- 実装/検証コストの低下
- 挙動の説明可能性が向上（ユーザー視点で理解しやすい）

## 18. 方針更新（性能最適化・2026-02-22 再確定）

最終更新: 2026-02-22

### 18.1 今回の確定事項

1. 円描画の再計算タイミング最適化（実装する）
   - 方針: 「秒が変わった時のみ」リング計算を更新する
   - ただし、以下のイベント時は即時再計算してUI差分を遅延させない
     - Split追加
     - チェックON/OFF変更
     - ラップ選択変更
     - セッション切替
   - 目的: 見た目（押し出し表示）を維持したまま計算回数を削減

2. ラップ表示秒キャッシュキーの軽量化（実装する）
   - 方針: `activeLapIDs.sorted` / `laps.map` を毎描画で組み立てる方式をやめる
   - 変更案: 版数（バージョン）ベースの軽量キーに置換
   - 要件: UI見た目・動作は変更しない

3. 更新FPSを running 時も 1秒に統一（実装する）
   - 方針: foreground/background やリングON/OFFに関わらず、running時の `clock` 更新を `1.0s` に統一
   - 許容差: 滑らかさ低下は許容（秒ベース表示優先）

4. チェックモード時の `selectedLapID` 正規化は未対応（今回は見送る）
   - 理由:
     - 秒配布対象は `activeLapIDs` であり、配布の正しさは保持される
     - 要件として「ラジオ切替時に最後の選択を使う」を優先
   - 補足:
     - チェックモードで未チェックLapが `selectedLapID` になり得るが、配布対象にはならない
     - 将来的に操作意図とのズレが問題化した場合に再検討

5. JSON保存は常時 compact 出力へ統一（実装する）
   - 方針: `prettyPrinted` / `sortedKeys` を使わず、常時本番寄せ出力にする
   - 影響:
     - 永続保存仕様・保存内容・UI/動作は不変
     - 1回あたりのエンコード/書き込みコストを削減

6. 設定モーダルの時間プレビュー追従は現状維持（今回は未対応）
   - 理由: 例示用途であり優先度が低い

7. 未使用関数は削除（実装する）
   - 対象: `activeTimelineOffset`（現状未参照）
   - 目的: 保守性向上とノイズ削減

### 18.2 実装順

1. running時FPSを1秒に統一
2. リング再計算を「秒更新 + 状態変更イベント時」に限定
3. ラップ表示秒キャッシュキー軽量化
4. JSON出力をcompact化
5. 未使用関数削除
...
## 20. 方針更新（Split分配モードのセッション個別化・2026-03-15）

最終更新: 2026-03-15

### 20.1 背景

- 現在の `Split分配モード`（`radio` / `checkbox`）はアプリ全体で1つだけ設定する方式になっている
- しかし実運用では、セッションごとに適した分配方式が異なり、全体共通設定だと切替コストが高い
- このため、分配モードは「各セッションが個別に持つ状態」へ移す

### 20.2 今回の確定方針

1. 各セッションが `Split分配モード` を保持する
   - `radio` / `checkbox` はセッション単位で保持する
   - セッション保存データへ含め、再起動後も復元する

2. アプリ全体設定からは「現在の分配モード」を削除する
   - 設定モーダルから現在の `Split分配モード` の切替UIは削除する
   - `AppSettings` / `AppSettingsStore` にある全体適用の `splitAccumulationMode` は削除対象

3. ただし「新規セッション作成時のデフォルトモード」はアプリ設定として追加する
   - 新規セッションの初期モードを設定できるようにする
   - 既定値は `radio`
   - 未保存時も `radio` を使う

4. セッション個別の切替UIを詳細ヘッダーへ追加する
   - 配置場所は「サマリーボタンの左側」
   - UIは `ラジオ | チェック` の2択セグメント風カプセル
   - 選択中の項目のみ背景色を付け、押すと即時切り替える

5. 既存セッションデータにモード情報がない場合は `checkbox` として復元する
   - 既存データ互換では「セッション個別モードの欠落値 = `checkbox`」
   - 新規セッション作成デフォルトの欠落値 = `radio`
   - 復元互換と新規既定値は役割が異なるため、別ルールで扱う

### 20.3 実装方針

1. セッション状態モデルへモード追加
   - `StopwatchService.SessionContext` に `splitAccumulationMode` を追加
   - 永続化用 `PersistedSessionContext` にも同項目を追加

2. `StopwatchService` のモード参照を「選択中セッションの mode」へ切替
   - `finishLap`
   - `selectLap`
   - `toggleLapActive`
   - 秒配分
   - `distributionCursor` 補正
   - 正規化処理
   これらをグローバル状態ではなく、対象セッションの mode を参照するようにする

3. 新規セッション作成時はアプリ設定のデフォルトモードを採用する
   - `radio` を既定値として設定UIから変更可能にする
   - 既存セッションの mode は上書きしない

4. 設定モーダルは「現在の mode」ではなく「デフォルト mode」を扱う
   - ここで変更されるのは今後作るセッションのみ

5. `SessionPopoverView` のヘッダーにセッション個別 mode 切替UIを追加する
   - サマリーボタン左へ配置
   - 切替時は即時反映し、保存も行う

### 20.4 テスト方針

- 新規セッションは設定されたデフォルト mode で作られる
- セッションA/Bで異なる mode を保持しても切替時に混ざらない
- 保存復元後も各セッションの mode が維持される
- 旧保存データで mode 欠落時は `checkbox` 復元になる
- mode 切替時に `activeLapIDs` / `distributionCursor` の整合が壊れない

## 21. 方針追加（ショートカット対応・2026-03-15）

最終更新: 2026-03-15

### 21.1 背景

- 作業開始/停止などを素早く行うためにショートカット対応を追加したい
- ただしショートカット実行時に、実際に処理されたことが視覚的に分かる必要がある
- そのため、処理実行時にPopoverを一時表示し、成功を見た目で伝える案を採用する

### 21.2 今回の確定方針

1. ショートカットはグローバル・アプリ内の両方で有効にする
   - SplitLog の Popover を開いていない状態でも反応する
   - アプリ表示中も同じショートカットで動作する

2. ショートカット実行時、Popover が閉じている場合のみ一時表示する
   - 処理実行後に Popover を表示する
   - 表示時間は 3 秒

3. 一時表示中にユーザー操作が入った場合は自動クローズをキャンセルする
   - クリックなどの操作が行われたら時間経過では閉じない
   - 「ショートカット成功の通知」から「通常の手動操作状態」へ遷移させる

4. すでに Popover が開いている場合は表示状態を変えない
   - ショートカット時は処理だけ行う
   - 既存の表示を閉じたり、再表示したりはしない

### 21.3 実装方針

1. `MenuBarController` をショートカット処理の窓口にする
   - グローバルショートカット登録
   - ショートカット発火時のアクション実行
   - Popover の一時表示/自動クローズ管理
   をここへ集約する

2. グローバルショートカットは macOS 標準寄りの方法で実装する
   - `Carbon` の `RegisterEventHotKey` 系を第一候補とする
   - アプリ内ショートカットも同じアクションへ流す

3. Popover の一時表示状態を明示的に管理する
   - 「ショートカット起因で開いたか」
   - 「自動クローズ予約が有効か」
   を区別できる状態を持つ

4. 自動クローズは 3 秒のタイマーで制御する
   - ショートカット起因で表示した時だけ予約する
   - ユーザー操作が入ったら予約を破棄する

5. Popover 内のユーザー操作を検知して自動クローズを止める
   - クリック等の入力を検知して、一時表示モードを解除する
   - 既存の通常操作には影響を与えないようにする
...
- ショートカットの基本修飾キーは `⌘⌃` とする
- 一般的な `⌘` 系ショートカットとの衝突を避けつつ、`⌃⌥⌘` ほど重くしない
...
2. メモ表示
   - `⌘⌃M` では現在選択中 Split のメモを開く
   - Popover が閉じている場合でも開き、そのままメモモーダルまで表示する

3. ラップ番号ショートカット
   - `⌘⌃1` ... `⌘⌃9` は上から1つ目〜9つ目のラップを対象にする
   - `⌘⌃0` は一番下の最新ラップを対象にする

4. ラップ操作時の mode ごとの挙動
   - `radio` の場合は対象ラップへ切り替える
   - `checkbox` の場合は対象ラップのチェックをトグルする
   - `checkbox` の場合、未チェックならチェックを入れ、チェック済みなら外す
...
- `Split` / `停止` / `再開` / ラップ操作 / メモ表示 などでショートカット実行した場合
  - Popover が閉じていれば、一時表示して成功を見せる
  - 一時表示は 3 秒
  - その間にユーザー操作が入ったら自動クローズをキャンセルする
